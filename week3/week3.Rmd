...
for (i in 1:df_size) {
    sum_of_trait <- trait_df[i,trait] + sum_of_trait
} 

if (sum_of_trait == df_size) { 
    cat ("The ancestral trait is:", colnames(trait_df)[trait])
    break
} else { 
      trait <- trait + 1 
      sum_of_trait <- 0
...
```
```{r Determine ancestral organism, echo = FALSE}

organism <- 1  
sum_of_orgtrait <- 0 
i <- 1

while (organism <= df_size) {

for (i in 1:df_size) {
    sum_of_orgtrait <- trait_df[organism, i] + sum_of_orgtrait
} 

if (sum_of_orgtrait == 1) { 
    cat ("The ancestral organism is:", rownames(trait_df)[organism])
    break
} else { 
      organism <- organism + 1 
      sum_of_orgtrait <- 0
